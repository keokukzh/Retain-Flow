...
// no local state needed; controlled by Swiper
...
export default function Testimonials() {
  // Swiper handles active index; we don't need local state for rendering
              ...
              <SwiperSlide key={t.id}>
                <div className="bg-gradient-to-br from-primary-50 to-blue-50 rounded-2xl p-8 md:p-12 relative overflow-hidden shadow">
                  ...
                  <div className="relative z-10">
                    ...
                    <blockquote className="text-xl md:text-2xl text-gray-800 text-center mb-8 leading-relaxed">&ldquo;{t.content}&rdquo;</blockquote>
                    <div className="flex items-center justify-center mb-8">
                      <div className="flex items-center">
                        ...
                        <div className="text-left">
                          ...
                          <div className="text-gray-600">{t.role} at {t.company}</div>
                          <div className="flex items-center mt-1">
                            <span className="text-sm text-primary-600 font-medium">{t.platform}</span>
                          </div>
                        </div>
                      </div>
                    </div>
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                      {t.results.map((r, i) => (
                        <div key={i} className="text-center">
                          <div className="text-3xl font-bold text-primary-600 mb-1">{r.value}</div>
                          <div className="text-sm text-gray-600 mb-1">{r.metric}</div>
                          <div className="text-xs text-green-600 font-medium">{r.improvement}</div>
                        </div>
                      ))}
                    </div>
                  </div>
